...
static int total_jmax = 0, total_avg_jmax = 0;
...
static int write_test_mode(struct device *dev, u8 type)
{
	...
	case U0_M2_RAWDATA_TEST:
		...
		break;
	case JITTER_TEST:
		testmode = BLU_JITTER_TEST_DC_OFFSET_FILTER_CMD;
		waiting_time = 200;
		break;
	}
...
static void BLU_jitter_enable(struct device *dev)
{
	...
	queue_delayed_work(ts->wq, &d->BLU_jitter_work,
		msecs_to_jiffies(10));
}
				...
				int* max)
{
	...
	for ( i=0; i<JITTER_TEST_CNT; i++) {
		...
		for (j=0; j<2; j++) {
			if (EVEN == j)
				frame_cmd = m2_odd_frame;
			else if (ODD == j)
				frame_cmd = m2_even_frame;
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	jitter_ret = prd_jitter_test(dev, JITTER_TEST, &max[0]);
	...
	if (0 == jitter_ret) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Jitter Result : Pass (%3d)\n", max[JITTER_VA]);
		TOUCH_I("Jitter Result : Pass (%3d)(%3d)\n", max[JITTER_VA], max[JITTER_AVG_VA]);
	} else {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Jitter Result : Fail (%3d)\n", max[JITTER_VA]);
