 ...
 */
 ...
 /* Be Included by base charger driver here and modify conditional build options.
 * : Refer to ../Makefile
 */

/* MIN/MAX thresholds for charging control
 */
#define SHOWCASE_SOC_MAX 45
#define SHOWCASE_SOC_MIN 50
...
static struct wake_lock showcase_charging_wakelock;
...
static void showcase_charging_block(void)
{
	...
	if( !wake_lock_active(&showcase_charging_wakelock) ) {
		wake_lock(&showcase_charging_wakelock);
	}
...
static void showcase_charging_release(void)
{
	...
	if( wake_lock_active(&showcase_charging_wakelock) ) {
		wake_unlock(&showcase_charging_wakelock);
	}
}
...
void qpnp_lbc_main_probe_lge(struct spmi_device *spmi)
{
	...
	wake_lock_init(&showcase_charging_wakelock, WAKE_LOCK_SUSPEND,
		"showcase_charging_wakelock");
}
